...
- `plate_base.stl` — основа 180 x 90 x 4 мм с задним глухим держателем под
  композитную арматуру 12 мм и пазом под надпись. Внешний диаметр держателя:
  20 мм. Плавный переход с табличкой строится как единая оболочка держателя,
  а не как отдельное пересекающееся тело.
...
--transition-plate-overlap 0.8      # насколько переход входит в табличку
--transition-end-margin 0           # отступ перехода сверху/снизу; 0 = вся длина
